<template>
  <div>
    <section class="content">
      <!--<h4 style="text-align: left">任务列表 <span-->
      ...
      <el-table :data="tasks" v-loading="listLoading" class="center-block" style="margin-left: 20px" stripe>
    ...
    methods: {
      ...
      formatterAddress(row, column) {
        if (column.property == 'status') {
          ...
          return row.msgTime ? $.Data.formatDate(new Date(row.msgTime * 1000), 'yyyy-MM-dd hh:mm:ss') : '--';
        } else if (column.property == 'finishTime') {
          return row.finishTime ? $.Data.formatDate(new Date(row.finishTime * 1000), 'yyyy-MM-dd hh:mm:ss') : '--';
        } else {
