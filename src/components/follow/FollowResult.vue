<template>
  <div>
    <section class="content">
      ...
      <div class="content" style="margin-left: 10px" v-show="activeItem == 'result'">
    ...
    methods: {
      ...
      formatterAddress(row, column) {
        if (column.property === 'sex') {//0-女  1-男  2-未知
          ...
          return row.upTime ? $.Data.formatDate(new Date(row.upTime * 1000), 'yyyy-MM-dd hh:mm:ss') : '--';
        } else if (column.property === 'originalUpTime') {
          return row.originalUpTime ? $.Data.formatDate(new Date(row.originalUpTime * 1000), 'yyyy-MM-dd hh:mm:ss') : '--';
      ...
      getTaskDetail() {
        this.$post('/follow/get/' + this.taskId, {}).then((data) => {
          ...
          this.task.startStr = $.Data.formatDate(new Date(this.task.startDate * 1000), 'yyyy-MM-dd hh:mm:ss');
          this.task.endStr = $.Data.formatDate(new Date(this.task.endDate * 1000), 'yyyy-MM-dd hh:mm:ss');
        }).catch((err) => {
        });
      }
    },
