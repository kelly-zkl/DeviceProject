<template>
  <div>
    <section class="content">
      <el-row style="margin: 20px 0 10px 0">
        <el-col :span="18" align="left">
          <el-input placeholder="终端imsi" v-model="query.imsi" :maxlength="30" size="medium"
                    style="width:250px;margin-top: 10px;margin-right: 10px"></el-input>
        ...
        <el-col :span="6" align="right">
          <el-button type="primary" icon="el-icon-plus" style="margin-top: 10px"
                     v-show="getButtonVial('bwTerminate:addBlack')" size="medium"
                     @click="runningStatusVisible = true;device={imsi: ''}">添加黑名单
          </el-button>
        </el-col>
      </el-row>
        ...
        <el-table-column align="left" label="操作" min-width="150" max-width="300">
          <template slot-scope="scope">
            <el-button @click.stop="deleteData(deviceList[scope.$index].imsi)" type="danger" style="margin: 5px 0"
                       v-show="getButtonVial('bwTerminate:deleteBlack')" size="small">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <!--添加黑名单-->
      <el-dialog title="添加黑名单" :visible.sync="runningStatusVisible" :width="dialogWidth">
        <el-form :model="device" :label-width="labelWidth" :rules="rules">
          <el-form-item label="黑名单" prop="imsi" required>
  ...
  export default {
    data() {
      let imsiValidator = (rule, value, callback) => {
        if (!numValid(value)) {
          callback(new Error("请输入正确的黑名单，只能输入数字"));
        } else {
          callback();
        }
      };
    ...
    methods: {
      ...
      addImsi() {
        ...
        this.$post("bwTerminate/addBlack", this.device, "添加成功").then(() => {
          this.getData();
          this.runningStatusVisible = false;
        });
      },
      deleteData(imsi) {
        this.$confirm('确定要删除该黑名单吗？', '提示', {
          type: 'warning'
        }).then(() => {
          this.$post("bwTerminate/deleteBlack", {"imsi": imsi}, "删除成功", "删除失败")
            .then(() => {
              this.getData();
            });
        }).catch(() => {
      ...
      getData() {
        ...
        this.$post("bwTerminate/queryBlack", this.query).then((data) => {
